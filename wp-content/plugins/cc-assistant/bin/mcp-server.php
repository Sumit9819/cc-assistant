...
function handle_tool_call( $tool_name, $arguments ) {
	// The local runner supplies an exact allowlist independently of model permissions.
	$automation = getenv( 'CC_AUTOMATION_ALLOWED_TOOLS' );
	if ( false !== $automation && '' !== $automation ) {
		$allowed = json_decode( $automation, true );
		if ( ! is_array( $allowed ) || ! in_array( $tool_name, $allowed, true ) ) { return array( 'isError' => true, 'content' => array( array( 'type' => 'text', 'text' => 'automation_tool_denied: this job phase does not authorize this tool.' ) ) ); }
		if ( 'draft_create_post' === $tool_name && empty( $arguments['dry_run'] ) ) {
			static $creation_attempted = false;
			if ( $creation_attempted || ( $arguments['post_type'] ?? '' ) !== 'post' || empty( $arguments['workflow_id'] ) ) { return array( 'isError' => true, 'content' => array( array( 'type' => 'text', 'text' => 'automation_create_limit: at most one workflow-bound post creation attempt per process. Inspect existing records; do not retry uncertain writes.' ) ) ); }
	...
	switch ( $tool_name ) {
        ...
        case 'refresh_publish_proposal':
            return format_tool_result( wp_rest_call( '/content/publication/refresh', 'POST', $arguments ) );
        ...
        case 'get_content_authors':
            return format_tool_result( wp_rest_call( '/content/authors' ) );
        case 'record_external_research':
            return format_tool_result( wp_rest_call( '/content/external-research', 'POST', $arguments ) );
		...
		case 'widget_schema':
			...
			if ( isset( $arguments['post_id'] ) ) { $params['post_id'] = (int) $arguments['post_id']; }
			if ( isset( $arguments['widget_id'] ) ) { $params['widget_id'] = (string) $arguments['widget_id']; }
