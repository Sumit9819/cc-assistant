...
function cc_brain_bridge_check( array $site_hashes, $bin_dir = null ) {
	...
	$local   = array();
	foreach ( array( 'browser-transport.mjs', 'browser/package.json', 'automation-runner.py', 'wcag/axe-run.js', 'wcag/form-observation.js', 'wcag/package.json' ) as $name ) {
...
function cc_brain_tool_push( callable $rest, $project_dir = null, $home = null, $chunk_bytes = 900000, $paths = null ) {
	...
	$responses = array();
	$expected_fingerprint = $site['fingerprint'] ?? '';
	foreach ( $chunks as $i => $chunk ) {
		$body = array( 'mode' => 'merge', 'files' => $chunk, 'pushed_from' => $from, 'expected_fingerprint' => $expected_fingerprint );
		if ( 0 === $i && ! empty( $delete ) ) {
			$body['delete'] = $delete;
		}
		...
		if ( ! isset( $r['fingerprint'] ) || ! is_string( $r['fingerprint'] ) ) { return array( 'error' => 'push_basis_missing', 'message' => 'The site did not return a new fingerprint; stop instead of guessing the next chunk basis.' ); }
	...
	try {
		...
		if ( 'json' !== $ext ) {
			if ( in_array( $ext, array( 'js', 'mjs' ), true ) ) {
				if ( ! @rename( $tmp, $tmp . '.' . $ext ) ) { return 'could not stage JavaScript'; }
				$tmp .= '.' . $ext;
			}
			$command = in_array( $ext, array( 'js', 'mjs' ), true ) ? array( getenv( 'CC_NODE_BINARY' ) ?: 'node', '--check', $tmp ) : array( PHP_BINARY, '-n', '-l', $tmp );
			if ( 'py' === $ext ) { $command = array( getenv( 'CC_PYTHON_BINARY' ) ?: 'python', '-c', 'import pathlib,sys; compile(pathlib.Path(sys.argv[1]).read_bytes(), sys.argv[1], "exec")', $tmp ); }
...
function cc_brain_tool_pull( callable $rest, $what = 'all', $mode = 'missing_only', $project_dir = null, $home = null ) {
	...
	if ( 'bridge' === $what ) {
		...
		foreach ( $r['files'] as $name => $e ) {
			if ( ( ! preg_match( '/^[A-Za-z0-9_.-]+\.php$/', (string) $name ) && ! in_array( $name, array( 'browser-transport.mjs', 'browser/package.json', 'automation-runner.py', 'wcag/axe-run.js', 'wcag/form-observation.js', 'wcag/package.json' ), true ) ) || ! is_array( $e ) || ! isset( $e['content'] ) ) {
				continue;
			}
			$target = $bin . '/' . $name;
