...
def call(name,a):
    if name=='get_content_authors':return {'authors':[{'id':4,'display_name':'Workshop Team'},{'id':9,'display_name':'admin-login'}],'default_author':None,'next_step':'Use the existing Workshop Team organization author per operator instruction; no invented credentials.'}
    if name=='content_decision_history' and scenario=='provider_context':return {'assessment':'external_provider_observations','provider':'ubersuggest','provenance':'agent_transcribed_from_connected_mcp; server_did_not_call_or_authenticate_provider_response','location':'Dallas TX','language':'English','observations':[{'subject':'bicycle puncture preparation','estimated_volume':0,'provider_age_days':190,'freshness_review':'older_than_90_days_review_applicability'},{'subject':'recurring flat tire','freshness_review':'update_date_unknown'}],'overlap_domains':['unrelated-business.invalid','local-workshop.invalid'],'limits':['Overlapping domains have not been verified as local business competitors.','GSC is unavailable.','No ranking or global originality claim is established.']}
    ...
    if name=='draft_create_post':
        if a.get('author_id')!=4:return {'error':'content_author_required','message':'Read current public authors and pass author_id=4 for the existing Workshop Team organization author.'}
