...
// Essential Addons bumps a view counter on every front-end render, so the
// loopback fetch that gathers evidence invalidates its own receipt. That
// deadlocked published pages: the fingerprint must ignore the counter, and
// the per-key digests must still name it when something does move.
update_post_meta(1,'_eael_post_view_count','1');
$with_view=CC_Assistant_Integrity::post_hash(1);
update_post_meta(1,'_eael_post_view_count','2');
check(CC_Assistant_Integrity::post_hash(1)===$stable && $with_view===$stable, 'A render-incremented view counter does not change the content fingerprint');
$moved=CC_Assistant_Integrity::meta_hash_diff(
    CC_Assistant_Integrity::state_meta_hashes($state),
    CC_Assistant_Integrity::state_meta_hashes(array('fields'=>$state['fields'],'terms'=>$state['terms'],'meta'=>array('rank_math_title'=>array('Moved'))+$state['meta']))
);
check(isset($moved['rank_math_title']) && !isset($moved['_eael_post_view_count']), 'Per-key digests name a real moved key and skip volatile ones');
