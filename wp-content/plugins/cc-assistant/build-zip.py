...
def main():
    required = ["bin/mcp-server.php", "bin/browser-transport.php", "bin/browser-transport.mjs", "bin/browser/package.json", "bin/automation-runner.py", "bin/agent-contract.php", "bin/tool-catalog.php", "bin/wcag/axe-run.js", "bin/wcag/form-observation.js", "bin/wcag/package.json"]
    missing = [name for name in required if not os.path.isfile(os.path.join(PLUGIN_DIR, name))]
    if missing:
        raise SystemExit("REFUSING TO BUILD: missing runtime files: " + ", ".join(missing))
    check_no_bom()
    check_php_syntax()
