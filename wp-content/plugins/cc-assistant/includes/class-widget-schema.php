...
class CC_Assistant_Widget_Schema {
	...
	public static function control_visible( $control, $values ) {
		...
		foreach ( $control['condition'] as $ckey => $cvalue ) {
			...
			// A multi-select control (submit_actions, motion_fx_devices) stores an
			// ARRAY, and Elementor then asks whether the required value is IN it.
			// Comparing the whole array to a scalar makes every such gate read as
			// off, which marked correct email settings inert and refused the queue.
			if ( is_array( $instance ) ) {
				$contains = is_array( $cvalue )
					? (bool) array_intersect( $cvalue, $instance )
					: in_array( $cvalue, $instance, true );
			} else {
	...
	public static function blocking_error( $warnings ) {
		...
		// The findings ride in the error data, but an apply run through wp-admin
		// shows only the message, so the operator saw "the schema rejects this"
		// with no way to learn WHICH key. Name them in the sentence itself: a
		// refusal nobody can act on costs a whole round trip to diagnose.
		$named = array();
		foreach ( $blocked as $w ) {
			$key = $w['setting'] ?? $w['key'] ?? '';
			if ( '' === $key && preg_match( '/"([^"]+)"/', (string) ( $w['message'] ?? '' ), $m ) ) { $key = $m[1]; }
			$label = ( '' !== $key ? $key : '?' ) . ' (' . ( $w['code'] ?? 'blocked' ) . ')';
			if ( ! in_array( $label, $named, true ) ) { $named[] = $label; }
		}
		$detail = $named ? ' Blocked: ' . implode( ', ', array_slice( $named, 0, 12 ) )
			. ( count( $named ) > 12 ? ' and ' . ( count( $named ) - 12 ) . ' more' : '' ) . '.' : '';
		return new WP_Error( 'unverified_elementor_settings', 'Settings were not queued: the live control schema rejects or cannot verify this change. Read widget_schema, fix the plan, then retry.' . $detail, array( 'status' => 422, 'findings' => $blocked ) );
	}
