# -*- coding: utf-8 -*-
"""Bridge import of blog08_tree.json into post 4023 (Blog #4, mini excavator chooser).
...
tree = json.load(open(os.path.join(HERE, "blog08_tree.json"), encoding="utf-8"))
r = call("import_elementor_data", {
    "post_id": 4032,
    "raw_data": json.dumps(tree, ensure_ascii=False),
    "enforce_bold_headings": False,
    "summary": "[BLOG8 - STEP 5 of 7] Full page design: Wheel Loader vs Skid Steer",
    "reasoning": ("Complete Elementor design for Blog #8, the final post in the calendar, built from the proven blog "
        "scaffold with layout v2 and the sticky sidebar baked in. Title leads with the comparison rather than "
        "'compact wheel loader', the phrase the category page ranks for, so the post feeds both category pages "
        "instead of competing with either; both are linked by exact anchor. Figures verified today off the live "
        "machine pages: mini skid steers 1,000-1,300 lb and 34.5-36 in wide from $27,999 CAD; wheel loaders 4,500 lb "
        "and up from $68,999, machine weight from 11,464 lb; TL5500 5,500 lb / 199 in dump / $73,999."),
})
