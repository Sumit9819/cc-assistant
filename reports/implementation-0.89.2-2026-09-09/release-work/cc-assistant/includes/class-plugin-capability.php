...
class CC_Assistant_Plugin_Capability {
    ...
    public static function inspect( $args ) {
        ...
        if ( 'sg-cachepress' === $slug ) {
            require_once __DIR__ . '/class-siteground-adapter.php';
            $out['native_adapter'] = CC_Assistant_SiteGround_Adapter::capability();
            $class = 'SiteGround_Optimizer\\Rest\\Rest';
            $out['native_toggle_routes'] = class_exists( $class ) && property_exists( $class, 'toggle_options' ) ? array_values( (array) $class::$toggle_options ) : array();
