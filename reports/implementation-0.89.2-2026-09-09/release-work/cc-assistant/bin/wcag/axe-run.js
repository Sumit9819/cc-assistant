...
async function keyboardChecks(page) {
  ...
  // Forms: submit empty and see whether an error is exposed to assistive
  // tech (aria-invalid / role=alert / aria-live text) or at least by native
  // constraint validation. "none" means a blind user gets silence.
  try {
    ...
    for (let f = 0; f < Math.min(formCount, 3); f++) {
      const res = await page.evaluate(async (idx) => {
        ...
        // A form with no required fields would actually submit (search forms navigate, contact
        // forms would send an empty message). Only probe forms whose empty submit is blocked by
        // constraint validation, and block navigation anyway as a safety net.
        if (!required.length) return { form: idx, fields: inputs.length, required: 0, result: 'not_probed_no_required_fields' };
        const native = !form.noValidate && !form.checkValidity();
        form.addEventListener('submit', ev => ev.preventDefault(), { capture: true });
        const btn = form.querySelector('button[type=submit], input[type=submit], button:not([type])');
        if (btn) { try { btn.click(); } catch (e) {} }
        await new Promise(r => setTimeout(r, 900));
        const invalid = form.querySelectorAll('[aria-invalid="true"]').length;
        const alerts = [...form.querySelectorAll('[role=alert], [aria-live]'), ...document.querySelectorAll('.elementor-message-danger, .elementor-message-error, .wpcf7-not-valid-tip, .gform_validation_errors')].filter(el => (el.textContent || '').trim().length > 0).length;
        return { form: idx, fields: inputs.length, required: required.length, result: invalid > 0 || alerts > 0 ? 'error_announced' : (native ? 'native_validation' : 'none') };
      }, f);
      if (res) out.forms.push(res);
    }
