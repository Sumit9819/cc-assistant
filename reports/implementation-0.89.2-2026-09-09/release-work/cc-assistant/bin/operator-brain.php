...
function cc_brain_bridge_check( array $site_hashes, $bin_dir = null ) {
	...
	$local   = array();
	foreach ( array( 'browser-transport.mjs', 'browser/package.json', 'automation-runner.py' ) as $name ) {
...
function cc_brain_tool_pull( callable $rest, $what = 'all', $mode = 'missing_only', $project_dir = null, $home = null ) {
	...
	if ( 'bridge' === $what ) {
		...
		foreach ( $r['files'] as $name => $e ) {
			if ( ( ! preg_match( '/^[A-Za-z0-9_.-]+\.php$/', (string) $name ) && ! in_array( $name, array( 'browser-transport.mjs', 'browser/package.json', 'automation-runner.py' ), true ) ) || ! is_array( $e ) || ! isset( $e['content'] ) ) {
				continue;
			}
			$target = $bin . '/' . $name;
