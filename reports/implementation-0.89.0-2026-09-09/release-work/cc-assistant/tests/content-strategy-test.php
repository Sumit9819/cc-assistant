...
check( 'all content routes have explicit permissions', 11 === count( $GLOBALS['routes'] ) && count( array_filter( $GLOBALS['routes'], static function ( $route ) { return ! empty( $route['permission_callback'] ); } ) ) === count( $GLOBALS['routes'] ) );
