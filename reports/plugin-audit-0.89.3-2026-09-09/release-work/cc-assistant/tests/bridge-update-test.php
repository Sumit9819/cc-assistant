...
$r=pull_fixture('function broken(', 'replace', null, 'wcag/axe-run.js');
check_bridge(count($r['errors'])===1 && !is_file($bin.'/wcag/axe-run.js'),'Invalid accessibility JavaScript is rejected before installation');
foreach(array('wcag/axe-run.js','wcag/form-observation.js','wcag/package.json') as $name) {
    $content=file_get_contents(dirname(__DIR__).'/bin/'.$name);
    $r=pull_fixture($content,'replace',null,$name);
    check_bridge(!$r['errors'] && file_get_contents($bin.'/'.$name)===$content,'Accessibility runtime asset is distributed: '.$name);
}
// Clean only fixed test files in this freshly created random directory.
foreach(array('fixture.php','fixture.php.previous','browser-transport.mjs','browser/package.json','automation-runner.py','wcag/axe-run.js','wcag/form-observation.js','wcag/package.json') as $file) { unlink($bin.'/'.$file); }
rmdir($bin.'/wcag'); rmdir($bin.'/browser'); rmdir($bin); rmdir(dirname($bin)); rmdir($root.'/wp-content/plugins'); rmdir($root.'/wp-content'); rmdir($root);
