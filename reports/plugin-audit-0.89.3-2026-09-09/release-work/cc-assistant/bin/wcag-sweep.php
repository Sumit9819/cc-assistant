...
function cc_wcag_browser_get( $url, $timeout = 30 ) {
	...
	if ( function_exists( 'curl_init' ) ) {
		$ch = curl_init( $url );
		curl_setopt_array( $ch, array( CURLOPT_RETURNTRANSFER => true, CURLOPT_FOLLOWLOCATION => false, CURLOPT_TIMEOUT => $timeout, CURLOPT_USERAGENT => $ua, CURLOPT_SSL_VERIFYPEER => true, CURLOPT_SSL_VERIFYHOST => 2 ) );
		if ( defined( 'CURLSSLOPT_NATIVE_CA' ) && '' === (string) ini_get( 'curl.cainfo' ) ) { curl_setopt( $ch, CURLOPT_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA ); }
		$body = curl_exec( $ch );
		$code = (int) curl_getinfo( $ch, CURLINFO_RESPONSE_CODE );
		curl_close( $ch );
		return array( 'code' => $code, 'body' => (string) $body );
	}
	$ctx  = stream_context_create( array( 'http' => array( 'timeout' => $timeout, 'header' => "User-Agent: $ua\r\n", 'follow_location' => 0 ), 'ssl' => array( 'verify_peer' => true, 'verify_peer_name' => true ) ) );
...
function cc_wcag_summarize( array $data ) {
	...
	$kb = array( 'pages_checked' => 0, 'skip_link_missing' => 0, 'skip_link_target_missing' => 0, 'focus_invisible_pages' => 0, 'focus_invisible_examples' => array(), 'forms_silent' => array(), 'forms_not_tested' => array(), 'popup_traps' => array() );
	foreach ( $ok as $p ) {
		...
		foreach ( (array) ( isset( $k['forms'] ) ? $k['forms'] : array() ) as $f ) {
			if ( 'submission_not_tested' === ( $f['result'] ?? '' ) && count( $kb['forms_not_tested'] ) < 10 ) {
				$kb['forms_not_tested'][] = array( 'url' => (string) $p['url'], 'form' => $f['form'], 'reason' => 'Only passive form markup was inspected; submission and error announcements require a controlled test.' );
			}
...
function cc_tool_wcag_sweep( $args ) {
	...
	return array(
		...
		'summary'   => $summary,
		'stored_at' => $store,
		'reading'   => 'Review critical/serious WCAG findings first. Contrast groups may identify shared template colours; vendor rules need inspection of the installed implementation. Keyboard focus checks are observations, not full compliance certification. Forms are inspected passively: forms_not_tested lists submission behavior that remains unknown. forms_silent is retained for older saved reports.',
	);
}
