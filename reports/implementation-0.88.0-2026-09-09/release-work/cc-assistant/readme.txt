...
No. Every change Claude proposes is queued in the Pending Changes inbox. You approve or reject each one.

= Can it edit theme or core files? =
No. The plugin only writes to its own database tables and its own folder. It cannot modify themes, core, uploads, or other plugins.

= Does it use my Anthropic API key? =
No. It connects through Claude Code, which uses your Claude subscription.
