...
class CC_Assistant_Content_Workflow {
	public static function prepare( $args = array() ) {
		...
		if ( 'new_blog' === $objective ) {
			...
			$steps[] = array( 'task' => 'Prepare the complete draft', 'tools' => array( 'list_categories', 'draft_create_post', 'get_post' ), 'completion' => 'Claude prepares the article, appropriate category, supported sources, metadata, useful internal links and available media. Read each tool schema. Save a draft and return its actual ID; do not claim a proposed article is already created.' );
		} else {
