...
class CC_Assistant_Workflow_Verifier {
    ...
    public static function verify( $args ) {
        ...
        if ( $pending_matches ) {
            $baseline = CC_Assistant_Integrity::baseline( (object) $pending );
            $evidence = $baseline['evidence'] ?? array();
            if ( empty( $evidence['environment_hash'] ) || empty( $evidence['posts'][$post_id]['post_hash'] ) ) {
                $out['issues'][] = 'Publication proposal lacks server-owned evidence for this draft. Read the current draft and refresh its existing publication proposal.';
            }
            if ( array( 'post_status' => 'publish' ) !== json_decode( (string) ( $pending['proposed_value'] ?? '' ), true ) ) {
                $out['issues'][] = 'Publication proposal payload does not match the supported publication intent.';
            }
            if ( isset( $baseline['publication_workflow_id'] ) && $baseline['publication_workflow_id'] !== $id ) {
                $out['issues'][] = 'Publication proposal and result are bound to different workflows.';
            }
