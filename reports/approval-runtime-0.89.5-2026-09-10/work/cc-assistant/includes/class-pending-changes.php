...
class CC_Assistant_Pending_Changes {
	...
	public static function queue( $args ) {
		...
		$args['pre_check_baseline'] = $baseline;
		// Rendering and validation can take seconds. The observed state must
		// still match after those reads, before storing or superseding any proposal.
		$proof = CC_Assistant_Evidence_Gate::validate_apply( (object) array( 'pre_check_baseline' => $baseline ) );
	...
	public static function mark_older_as_superseded( $new_id, $args ) {
		...
		$updated      = $wpdb->query( $wpdb->prepare(
			"UPDATE {$table} SET superseded_by = %d WHERE id IN ({$placeholders}) AND status = 'pending' AND superseded_by IS NULL",
			$sql_args
		) );
		...
		if ( $updated > 0 ) {
			...
			$rows    = $wpdb->get_results( $wpdb->prepare(
				"SELECT id, change_summary FROM {$table} WHERE id IN ({$sum_ph}) AND superseded_by = %d",
				array_merge( $ids_to_mark, array( (int) $new_id ) )
			) );
			$notices = array();
