...
CC Assistant 0.89.0 supports normal HTTPS REST requests and an optional browser transport. Both use a WordPress Application Password. Human approval uses a separate administrator browser session.
...
On 9 September 2026, the complete 0.89.0 staged bridge successfully read erofwhiterock.com through Chrome after direct HTTP received a SiteGround challenge. The site's backend reported **0.87.0**. The older local bridge was missing `browser-transport.mjs` and `browser/package.json`; both files are required. Copy the complete `bin` folder, including these assets. Install the current ZIP on WordPress for server-side fixes; changing the desktop transport does not update the live backend. A successful check does not guarantee permanent CAPTCHA clearance.
...
Version 0.89 shared-brain writes require an expected fingerprint and a server lock. Update older bridges before pushing. The optional Python runner is also included in bridge distribution and requires Python for syntax validation. See AUTOMATION.md for private configuration, budgets, job recovery and scheduling; installation does not enable paid background jobs.
