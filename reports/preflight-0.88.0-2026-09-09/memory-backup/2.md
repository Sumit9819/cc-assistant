...
3. **Check intent**: is the page's single intent clear? Is any other intent mixing in? (service = transactional/local; informational depth → spoke). See [[feedback_intent_doctrine]].
4. **Check the 2026 standard** end to end: run page_robustness_audit (intent → conversion → trust layers) and win_audit against same-intent competitors. Message-match H1, answer-first, one primary CTA, social proof, YMYL E-E-A-T (cited claims, mode-correct medical schema, NAP), extractability.
