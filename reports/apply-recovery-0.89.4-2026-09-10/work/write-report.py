...
manifest=load(report/'release-manifest.json')
initial=next(r['result']['pending'] for r in load(report/'initial-results.json') if r['name']=='list_pending_changes')
last=load(report/'final-live-checks-results.json')
assert all(not r.get('isError') for r in last)
pending=next(r['result']['pending'] for r in last if r['name']=='list_pending_changes')
assert len(pending)==28
assert {int(r['id']):r['proposed_value'] for r in pending}=={int(r['id']):r['proposed_value'] for r in initial},'Pending rows changed during this repair'
comparisons=load(root/'live-source-comparison.json')
assert len(comparisons)==18 and not any(r['unexpected_differences'] for r in comparisons)
text=f'''# CC Assistant 0.89.4 — approval recovery fix
...
'''
(report/'RECOVERY-REPORT.md').write_text(text,encoding='utf-8')
print(json.dumps({'report':str(report/'RECOVERY-REPORT.md'),'pending_count':len(pending),'same_proposal_ids_and_payloads':True,'unexpected_content_changes':False}))
