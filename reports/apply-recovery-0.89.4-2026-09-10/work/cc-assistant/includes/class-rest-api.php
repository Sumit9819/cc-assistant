...
class CC_Assistant_REST_API {
	...
	public static function register_routes() {
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => array( __CLASS__, 'check_permission' ),
				'args'                => array( 'author_id' => array( 'type' => 'integer', 'minimum' => 1 ) ),
			)
		);
	...
	public static function handle_whoami() {
		...
		$identity['session_recap'] = array(
			...
			'evidence_policy' => array( 'enforced_by' => 'WordPress shared proposal queue', 'ttl_seconds' => 600, 'scope' => 'WordPress user and Application Password/browser session', 'required_reads' => array( 'whoami', 'verified_page_audit for each published target', 'get_post(slim=false) for each draft/template', 'get_kit_settings for kit edits', 'get_plugin_settings for option edits' ), 'approval' => 'The observed post and plugin/theme/kit/SEO environment must still match at approval. One selected bulk approval can combine independent widget updates when exact predicted state is verified. Other edits or later requests require fresh evidence. After deployment and related source edits, use refresh_publish_proposal with a current workflow to refresh an existing blog publication intent; do not duplicate its draft.', 'unknown' => 'Missing evidence, CAPTCHA and unsupported controls block proposals. Stored values do not prove visible effects or Google indexing.' ),
	...
	public static function handle_draft_post_meta( WP_REST_Request $req ) {
		...
		if ( 'post_status' === $field ) {
			$valid = CC_Assistant_Apply::validate_field_status( $value );
	...
	public static function handle_draft_create_post( WP_REST_Request $req ) {
		...
		$dry_run = filter_var( $req->get_param( 'dry_run' ), FILTER_VALIDATE_BOOLEAN );
		if ( $dry_run ) {
			if ( in_array( $req->get_param( 'post_type' ) ?: 'page', array( 'post', 'page' ), true ) ) {
				require_once __DIR__ . '/class-content-authors.php';
				$author = CC_Assistant_Content_Authors::resolve( (int) $req->get_param( 'author_id' ) );
		...
		$args = array(
			...
			'author_id'               => (int) $req->get_param( 'author_id' ),
	...
	public static function handle_pre_publish( WP_REST_Request $req ) {
		...
		$result['publication_gate'] = CC_Assistant_Pre_Publish::evaluate_publish_gate( $id );
		$result['check_scope'] = 'The top-level pass and checks describe the advisory checklist. publication_gate is the quality gate used by publish_draft. Neither certifies current proposal evidence, approval, factual accuracy, browser output, indexing or ranking.';
